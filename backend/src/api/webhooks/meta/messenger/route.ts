...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  return verifyMetaWebhook(req, res, process.env.MESSENGER_VERIFY_TOKEN || process.env.META_VERIFY_TOKEN)
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const result = await inboxService.ingestMessengerWebhook(payload)
